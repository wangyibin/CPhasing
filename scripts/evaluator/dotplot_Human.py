...
def read_chrom(contig_size, cluster_contigs):
    ...
    for i in contig_size.keys():
        ...
        chrom = i.split(".")[0]
        if ("scaffold" not in chrom.lower()) and ("unloc" not in chrom.lower()):
...
def dotplot(cluster_data, contig_list, chrom_to_contig, output):
    ...
    for g in natsorted(new_K):
        k = list(filter(lambda x: ("scaffold" not in x.lower() and "unloc" not in x.lower()), new_K[g]))
        K.append(k)

    contig_list = list(filter(lambda x: ("scaffold" not in x.lower() and "unloc" not in x.lower()), contig_list))
