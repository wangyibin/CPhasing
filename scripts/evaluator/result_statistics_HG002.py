...
def parse_groups(groups, fa_len_dict):
    ...
    excluded_anchored_len = 0
    excluded_inter_homo_err = 0
    excluded_inter_nonhomo_err = 0
    ...
    for group in groups.values.tolist():
        ...
        for ctg in group:
            ...
            if source_chr.split('_')[0] not in {'chr13', 'chr14', 'chr15', 'chr21', 'chr22'}:
                excluded_group_len += length
        ...
        for source_chr, length in source_chr_len_list:
            ...
            if source_chr.split("_")[0] in {'chr13', 'chr14', 'chr15', 'chr21', 'chr22'} or source_chr == dominant_chr:
                continue 
            if source_chr != dominant_chr:
                if source_chr.split("_")[0] == dominant_chr.split("_")[0]:
...
def main():
    ...
    n_groups, anchored_len_dict, inter_homo_err, inter_nonhomo_err, largest_group_dict, excluded_anchored_len, excluded_inter_homo_err, excluded_inter_nonhomo_err = parse_groups(cluster_df, fa_len_dict)
    ...
    excluded_contiguity = sum([d_chr_len/anchored_len_dict[d_chr] for d_chr, d_chr_len in largest_group_dict.items() if d_chr.split('_')[0] not in {'chr13', 'chr14', 'chr15', 'chr21', 'chr22'}]) / len([source_chr for source_chr in anchored_len_dict if source_chr.split('_')[0] not in {'chr13', 'chr14', 'chr15', 'chr21', 'chr22'}])
    ...
    excluded_inter_homo_err_rate = excluded_inter_homo_err / excluded_anchored_len * 100 if excluded_anchored_len else 0
    excluded_inter_nonhomo_err_rate = excluded_inter_nonhomo_err / excluded_anchored_len * 100 if excluded_anchored_len else 0
