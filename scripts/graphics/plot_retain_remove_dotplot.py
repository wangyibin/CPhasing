...
def main(args):
    ...
    for ploidy, tmp_df in df.groupby('Sample'):
        ...
        sns.scatterplot(data=tmp_df, x="Retain", y="Remove", style=tmp_df['N50'], hue="Software",
        ...
        ax.set_xlabel("Proportion of retained \n $\mathit{cis}$ contacts", fontsize=16)
        ax.set_ylabel("Proportion of removed \n h-$\mathit{trans}$ contacts", fontsize=16)
        plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left' )
        ax.legend().remove()
        plt.savefig(f"{ploidy}.prune.dotplot.png", dpi=600, bbox_inches='tight')
        plt.savefig(f"{ploidy}.prune.dotplot.pdf", dpi=600, bbox_inches='tight')
